...
TypeScript conviviendo con `.jsx` (`allowJs: true`), acotado a esta carpeta y a `api.ts` — las páginas siguen sin type-checking. `Jugador.ts` y `Equipo.ts` son **clases** con métodos reales (`Jugador.nombreCompleto`, `Jugador.esCapitanDe(equipoId)`, `Equipo.tieneEscudo()`), no solo tipos planos; `types.ts` tiene las interfaces del resto de entidades (`TorneoDTO`, `PartidoDTO`, `CanchaDTO`, etc.).

**Actualizado (14/08)**: ya se usan en código de producción, no solo declarados. `InscribirEquipos.jsx` construye `new Equipo(equipo)` para resolver el escudo (`tieneEscudo()`/`escudoUrlCompleta(ASSETS_URL)`) en vez de armar la URL a mano; `RegistroSanciones.jsx` usa `new Jugador(s.jugador).nombreCompleto` en la búsqueda, la tabla y el modal de confirmación de borrado, en vez de concatenar `nombre`/`apellido` en 3 lugares distintos.
