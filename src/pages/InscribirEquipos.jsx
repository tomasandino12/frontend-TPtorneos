...
// Pantalla enfocada exclusivamente en inscribir/quitar equipos de este
// torneo. Árbitros, Canchas y Partidos (con la carga de resultados y la
// generación del fixture) se gestionan desde "Editar Torneo" — ver
// CrearTorneo.jsx en modoEdicion.
export default function InscribirEquipos() {
  ...
  async function loadData() {
    ...
    setPageError("");
    try {
      const [resTorneo, resEquipos] = await Promise.all([
        adminApiFetch(`/torneo/${torneoId}`),
        adminApiFetch("/equipos"),
      ]);

      const [dTorneo, dEquipos] = await Promise.all([
        resTorneo.json(),
        resEquipos.json(),
      ]);
